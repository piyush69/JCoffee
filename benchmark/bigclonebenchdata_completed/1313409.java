    ...
    public void setPilot(PilotData pilotData) throws UsernameNotValidException {
        try {
            ...
            if ((int)(Object)pilotData.id == 0) {
                ...
                psta =(PreparedStatement)(Object) jdbc.prepareStatement("INSERT INTO pilot " + "(name, address1, address2, zip, city, state, country, birthdate, " + "pft_theory, pft, medical, passenger, instructor, loc_language, " + "loc_country, loc_variant, username, password, id) " + "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,pilot_id_seq.nextval)");
            } else {
                MyHelperClass jdbc = new MyHelperClass();
