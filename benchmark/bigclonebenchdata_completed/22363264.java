...
class c22363264 {
public static MyHelperClass to64(StringBuilder o0, int o1, int o2){ return null; }
//public MyHelperClass to64(StringBuilder o0, int o1, int o2){ return null; }
    ...
    public static String md5Crypt(final byte[] key, final byte[] salt) throws NoSuchAlgorithmException {
        ...
        final MessageDigest _md =(MessageDigest)(Object) MessageDigest.getInstance("MD5");
        _md.update(key);
        MyHelperClass MAGIC = new MyHelperClass();
        _md.update((byte[])(Object)MAGIC.getBytes());
        _md.update(salt);
//        MyHelperClass MessageDigest = new MyHelperClass();
        final MessageDigest md2 =(MessageDigest)(Object) MessageDigest.getInstance("MD5");
        md2.update(key);
        md2.update(salt);
        md2.update(key);
...
//            MyHelperClass MessageDigest = new MyHelperClass();
            final MessageDigest md3 =(MessageDigest)(Object) MessageDigest.getInstance("MD5");
            if ((n & 1) != 0) {
                md3.update(key);
            } else {
                md3.update(abyFinal);
