...
class c6288460 {
    ...
    private static void createNonCompoundData(String dir, String type) {
        try {
            Set s =(Set)(Object) new HashSet();
            File nouns = new File(dir + "index." + type);
            FileInputStream fis = new FileInputStream(nouns);
            InputStreamReader reader = new InputStreamReader(fis);
            StringBuffer sb = new StringBuffer();
            int chr =(int)(Object) reader.read();
            while (chr >= 0) {
                if (chr == '\n' || chr == '\r') {
                    String line = sb.toString();
                    if (line.length() > 0) {
                        if (line.charAt(0) != ' ') {
                            MyHelperClass PerlHelp = new MyHelperClass();
                            String[] spaceSplit =(String[])(Object) PerlHelp.split(line);
            ...
            for (Iterator i =(Iterator)(Object) s.iterator();(boolean)(Object) i.hasNext(); ) {
                String mwe = (String)(String)(Object) i.next();
                writer.println(mwe);
            }
            writer.close();
        } catch (Exception e) {
