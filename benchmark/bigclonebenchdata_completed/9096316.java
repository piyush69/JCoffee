...
public MyHelperClass convertEventTimeIntoTimestamp(String o0){ return null; }
    ...
    private void handleNodeDown(long eventID, long nodeID, String eventTime) {
        ...
        try {
            ...
            PreparedStatement activeSvcsStmt =(PreparedStatement)(Object) dbConn.prepareStatement(OutageConstants.DB_GET_ACTIVE_SERVICES_FOR_NODE);
//            MyHelperClass OutageConstants = new MyHelperClass();
...
//            MyHelperClass OutageConstants = new MyHelperClass();
            ...
            if ((boolean)(Object)log.isDebugEnabled()) log.debug("handleNodeDown: creating new outage entries...");
            ...
            while ((boolean)(Object)activeSvcsRS.next()) {
                String ipAddr =(String)(Object) activeSvcsRS.getString(1);
                long serviceID =(long)(Object) activeSvcsRS.getLong(2);
                if ((boolean)(Object)openOutageExists(dbConn, nodeID, ipAddr, serviceID)) {
                    if ((boolean)(Object)log.isDebugEnabled()) log.debug("handleNodeDown: " + nodeID + "/" + ipAddr + "/" + serviceID + " already down");
                } else {
                    long outageID = -1;
                    ...
                    if ((boolean)(Object)log.isDebugEnabled()) log.debug("handleNodeDown: Recording outage for " + nodeID + "/" + ipAddr + "/" + serviceID);
                }
            }
            activeSvcsRS.close();
            try {
                dbConn.commit();
                if ((boolean)(Object)log.isDebugEnabled()) log.debug("Outage recorded for all active services for " + nodeID);
            } catch (UncheckedIOException se) {
                log.warn("Rolling back transaction, nodeDown could not be recorded  for nodeId: " + nodeID,(SQLException)(Object) se);
                try {
                    dbConn.rollback();
                } catch (UncheckedIOException sqle) {
            ...
            log.warn("SQL exception while handling \'nodeDown\'",(SQLException)(Object) sqle);
        } finally {
            try {
...
public MyHelperClass DB_OPEN_RECORD;
	public MyHelperClass DB_GET_ACTIVE_SERVICES_FOR_NODE;
	public MyHelperClass DB_INS_NEW_OUTAGE;
	public MyHelperClass NODE_DOWN_EVENT_UEI;
