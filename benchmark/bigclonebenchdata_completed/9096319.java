    ...
    private void handleNodeRegainedService(long eventID, long nodeID, String ipAddr, long serviceID, String eventTime) {
        ...
        try {
            ...
            if ((boolean)(Object)openOutageExists(dbConn, nodeID, ipAddr, serviceID)) {
                ...
                PreparedStatement outageUpdater =(PreparedStatement)(Object) dbConn.prepareStatement(OutageConstants.DB_UPDATE_OUTAGE_FOR_SERVICE);
                ...
                try {
                    dbConn.commit();
                    if ((boolean)(Object)log.isDebugEnabled()) log.debug("nodeRegainedService: closed outage for nodeid/ip/service " + nodeID + "/" + ipAddr + "/" + serviceID + " in DB");
                } catch (UncheckedIOException se) {
                    log.warn("Rolling back transaction, nodeRegainedService could not be recorded  for nodeId/ipAddr/service: " + nodeID + "/" + ipAddr + "/" + serviceID,(SQLException)(Object) se);
                    try {
                        dbConn.rollback();
                    } catch (UncheckedIOException sqle) {
...
public MyHelperClass NODE_REGAINED_SERVICE_EVENT_UEI;
	public MyHelperClass DB_UPDATE_OUTAGE_FOR_SERVICE;
